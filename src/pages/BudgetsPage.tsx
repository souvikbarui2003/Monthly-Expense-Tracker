...
import { localGetBudgets, localCreateBudget, localUpdateBudget, localDeleteBudget } from "../lib/localStore";
...
export default function BudgetsPage() {
  ...
  const convexBudgets = useQuery(api.budgets.list, USE_CONVEX && userId ? { userId: userId as any } : "skip");
  const createBudget = useMutation(api.budgets.create);
  const updateBudget = useMutation(api.budgets.update);
  const deleteBudget = useMutation(api.budgets.remove);

  const localBudgets = useMemo(() => !USE_CONVEX && userId ? localGetBudgets(userId) : null, [userId, refreshKey]);
  ...
  const handleAdd = async () => {
    ...
    try {
      if (USE_CONVEX) {
        if (editingBudget) {
          await updateBudget({ budgetId: editingBudget._id, userId: userId as any, name: form.name, amount });
        } else {
          await createBudget({ userId: userId as any, name: form.name, amount, periodType: form.period, startDate: start, endDate: end });
        }
      } else {
        if (editingBudget) {
          localUpdateBudget(editingBudget._id, userId!, { name: form.name, amount });
        } else {
          localCreateBudget({ userId: userId!, name: form.name, amount, periodType: form.period, startDate: start, endDate: end });
        }
        setRefreshKey((k) => k + 1);
      }
      ...
      setShowModal(false);
    } catch (err: any) { toast.error(err.message || "Failed to save budget"); }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      if (USE_CONVEX) { await deleteBudget({ budgetId: id as any, userId: userId as any }); }
      else { localDeleteBudget(id, userId!); setRefreshKey((k) => k + 1); }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{budgets?.length || 0} active budgets · {formatCurrency(totalBudget)} total</p>
        </div>
        <button onClick={() => { setEditingBudget(null); setForm({ name: "", amount: "", category: "", period: "monthly" }); setShowModal(true); }} className="flex items-center gap-1 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {budgets && budgets.length > 0 ? (
          budgets.map((budget: any) => {
            ...
            const status = getStatus(0, budget.amount);
            return (
              <div key={budget._id} className="rounded-xl border bg-card p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(budget)} className="rounded p-1 hover:bg-muted"><Edit3 className="h-3.5 w-3.5" /></button>
                    <button onClick={() => handleDelete(budget._id)} className="rounded p-1 hover:bg-destructive/10 hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs">
                  <div className={`flex items-center gap-1 ${status.color}`}><status.icon className="h-3.5 w-3.5" />{status.label}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-card p-6 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setShowModal(false); setEditingBudget(null); }} className="rounded p-1 hover:bg-muted"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium">Budget Name</label>
                <input type="text" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g., Food Budget" className="mt-1 flex h-10 w-full rounded-lg border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={form.period} onChange={(e) => setForm((f) => ({ ...f, period: e.target.value as any }))} className="mt-1 flex h-10 w-full rounded-lg border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">
                  <option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="semester">Semester</option><option value="custom">Custom</option>
                </select>
              </div>
